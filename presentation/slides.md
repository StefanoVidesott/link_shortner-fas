...
<!-- # I Due Pilastri dell'Osservabilità -->
# Segnali impiegati

<div class="grid grid-cols-3 gap-8 mt-8 max-w-2xl mx-auto">

<div v-click.fade.up.scale class="p-5 rounded-xl bg-gradient-to-b from-red-500/10 to-red-500/5 border border-red-500/20">
  <div class="text-red-400 font-bold text-lg mb-1">Metriche</div>
  <div class="text-slate-400 text-xs mb-3 font-mono">Prometheus + Grafana</div>
  <p class="text-sm text-slate-400 leading-relaxed">Valori numerici aggregati nel tempo. Rispondono a "quanto?" e "quanto spesso?".</p>
</div>

<div v-after.fade.up.scale class="p-5 rounded-xl bg-gradient-to-b from-green-500/10 to-green-500/5 border border-green-500/20">
  <div class="text-green-400 font-bold text-lg mb-1">Log</div>
  <div class="text-slate-400 text-xs mb-3 font-mono">Loki + Promtail</div>
  <p class="text-sm text-slate-400 leading-relaxed">Eventi discreti con contesto testuale. Rispondono a "cosa è successo esattamente?".</p>
</div>

<div v-after.fade.up.scale class="p-5 rounded-xl bg-gradient-to-b from-purple-500/10 to-purple-500/5 border border-green-500/20">
  <div class="text-purple-400 font-bold text-lg mb-1">Errori</div>
  <div class="text-slate-400 text-xs mb-3 font-mono">Sentry SDK</div>
  <p class="text-sm text-slate-400 leading-relaxed">Eccezioni con stack trace e contesto completo. Rispondono a "perché è andato storto?".</p>
</div>

</div>

<div v-click.fade.up.scale class="mt-8 p-4 rounded-lg bg-slate-800/50 border border-slate-700 text-left max-w-2xl mx-auto">
  <p class="text-sm text-slate-300">
    <span class="text-sky-400 font-semibold">Usati insieme:</span>
    le metriche identificano <span class="text-red-300">quando</span> e <span class="text-red-300">quanto</span> qualcosa va storto — i log spiegano <span class="text-green-300">cosa</span> è successo esattamente in quel momento.
  </p>
</div>
...
<div class="p-3 rounded-lg bg-slate-800/60 border border-red-500/20">
...
<div v-click.fade.up.scale class="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 text-slate-400 text-xs">
  <span class="text-sky-400 font-semibold">rate()</span> calcola la variazione media al secondo nella finestra temporale. Usare <code>[5m]</code> per dashboard stabili, <code>[1m]</code> per maggiore reattività.
</div>

<div v-click.fade.up.scale class="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 text-slate-400 text-xs">
  <span class="text-red-400 font-semibold">http_status=~"5.."</span> usa una regex — <code>=~</code> filtra tutti i codici che iniziano con 5 (500, 502, 503 …).
</div>
...
# Query PromQL — Latenza & Business

<div class="space-y-4 mt-3 text-sm">

<div class="p-3 rounded-lg bg-slate-800/60 border border-orange-500/20">
...
<div class="p-3 rounded-lg bg-slate-800/60 border border-teal-500/20">
  <div class="text-teal-400 text-xs font-semibold mb-1">Redirect rate per link creato</div>
...
<div class="p-3 rounded-lg bg-slate-800/60 border border-green-500/20">
  <div class="text-green-400 text-xs font-semibold mb-1">Cleanup heartbeat — Deadman Switch</div>
...
<div v-click.fade.up.scale class="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 text-slate-400 text-xs">
  <span class="text-orange-400 font-semibold">histogram_quantile(0.95)</span> — il 95% delle richieste è più veloce di questo valore. <code>le</code> è il label "less than equal" generato automaticamente dall'Histogram.
</div>

<div v-click.fade.up.scale class="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 text-slate-400 text-xs">
  <span class="text-teal-400 font-semibold">Redirect rate</span> — quante volte in media viene usato ogni link creato nell'intervallo. Misura l'utilità reale dei link.
</div>

<div v-click.fade.up.scale class="p-4 rounded-lg bg-green-500/10 border border-green-500/20 text-slate-400 text-xs">
  <span class="text-green-400 font-semibold">Deadman Switch</span> — se il valore supera <span class="text-red-400 font-semibold">1800s</span> (30 min) il worker di cleanup si è bloccato silenziosamente. Senza questa metrica l'anomalia sarebbe invisibile.
</div>
...
<div class="mt-6 p-8 rounded-xl border-2 border-dashed border-slate-600 bg-slate-800/30 text-slate-500 text-sm max-w-2xl mx-auto">
  <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mx-auto mb-3 text-slate-600" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Zm10.5-11.25h.008v.008h-.008V8.25Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>
  <p class="font-mono">[ screenshot · home page ]</p>
  <p class="text-xs mt-1 text-slate-600">Inserire screenshot dell'applicazione in esecuzione</p>
</div>
...
<div class="mt-6 p-8 rounded-xl border-2 border-dashed border-yellow-500/30 bg-yellow-500/5 text-yellow-600/60 text-sm max-w-2xl mx-auto">
  <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mx-auto mb-3 text-yellow-500/40" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 0 0 6 16.5h2.25M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0 1 18 16.5h-2.25m-7.5 0h7.5m-7.5 0-1 3m8.5-3 1 3m0 0 .5 1.5m-.5-1.5h-9.5m0 0-.5 1.5M9 11.25v1.5M12 9v3.75m3-6.75v6.75" /></svg>
  <p class="font-mono">[ screenshot · grafana dashboard ]</p>
  <p class="text-xs mt-1 text-yellow-500/30">Inserire screenshot della dashboard con dati reali</p>
</div>
